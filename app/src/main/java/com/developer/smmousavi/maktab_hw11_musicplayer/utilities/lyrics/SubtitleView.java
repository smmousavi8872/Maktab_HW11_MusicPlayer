package com.developer.smmousavi.maktab_hw11_musicplayer.utilities.lyrics;
...
public class SubtitleView extends android.support.v7.widget.AppCompatTextView implements Runnable {
  ...
  public void setSubSource(String lyricsText, String mime) {
    if (mime.equals(MediaPlayer.MEDIA_MIMETYPE_TEXT_SUBRIP))
      track = getSubtitleFile(lyricsText);
    else
  ...
  private TreeMap<Long, Line> getSubtitleFile(String lyricText) {
    InputStream inputStream = null;
    try {

      inputStream = new ByteArrayInputStream(lyricText.getBytes(StandardCharsets.UTF_8));
